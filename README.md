...
We implement a correction branch that goes in parallel with the prediction head (original backbone) of the model. It takes a latent representation of the input ($z_t$) and predicts an offset map ($c_t$) that corrects the output of the original prediction ($\hat{x}_{t}$). 
